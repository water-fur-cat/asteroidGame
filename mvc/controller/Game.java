				...
				if (pntFalCenter.distance(pntFloaterCenter) < (nFalRadiux + nFloaterRadiux)) {
					if (movFloater instanceof NewShipFloater) {
						switch (((NewShipFloater) movFloater).getKind()){
							case ADDLIFE:
								int num = CommandCenter.getInstance().getNumFalcons()+1;
								CommandCenter.getInstance().setNumFalcons(num);
								break;
							case POWERUP:
								CommandCenter.getInstance().getFalcon().setPowerUp(100);
								break;
							case SHIELD:
							...
							case BOMB:
								for (Movable movFoe : CommandCenter.getInstance().getMovFoes()){
									...
									CommandCenter.getInstance().getOpsList().enqueue(new Explosion((Asteroid)movFoe), CollisionOp.Operation.ADD);
									for (int i = 0; i < R.nextInt(5); i++){
										CommandCenter.getInstance().getOpsList().enqueue(new Debris((Asteroid)movFoe), CollisionOp.Operation.ADD);
									}
								}
								break;
						}
...
//			else{
//				CommandCenter.getInstance().getOpsList().enqueue(new Explosion(astExploded), CollisionOp.Operation.ADD);
//			}
//
		}
		...
		CommandCenter.setScore(CommandCenter.getScore() + 6/(((Asteroid)movFoe).getSize()+1));
	...
	private void spawnNewShipFloater() {
		...
		if (nTick % (SPAWN_NEW_SHIP_FLOATER - nLevel * 6) == 0) {
		...
		if (fal != null) {
			...
			case KILL:
				 if (CommandCenter.getNumKill() > 0 ) {
					 ...
					 CommandCenter.setNumKill(CommandCenter.getNumKill() - 1);
				 }
				 break;
			case SHIELD:
				 if (CommandCenter.getNumShield() > 0 ){
					 CommandCenter.getInstance().getFalcon().setProtected(true);
					 CommandCenter.setNumShield(CommandCenter.getNumShield() - 1);
				 }
				 break;
//			case CIRCLE:
//				if(CommandCenter.getNumCircle() > 0){
//					CommandCenter.setCircle();
//					CommandCenter.setNumCircle(CommandCenter.getNumCircle() - 1);
//				}
//				break;
			// case NUM_ENTER:
	...
	public void keyReleased(KeyEvent e) {
		...
		if (fal != null) {
			switch (nKey) {
			case FIRE:
				if(CommandCenter.getInstance().getFalcon().getPowerUp() > 0){
...
//				CommandCenter.getInstance().getOpsList().enqueue(new Bullet(fal), CollisionOp.Operation.ADD);
			...
			case SPECIAL:
				CommandCenter.getInstance().getOpsList().enqueue(new Cruise_2(fal), CollisionOp.Operation.ADD);
				//Sound.playSound("laser.wav");
				break;
